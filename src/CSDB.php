...
?><?php
	...
	class CSDB
	{
		...
		protected function AssertPDOAvailable($checkdb)
		{
			...
			{
				throw new \Exception(\CubicleSoft\CSDB::DB_Translate("The driver is not available."));
				exit();
			}

			if ($checkdb && $this->dbobj === false)
			{
				throw new \Exception(\CubicleSoft\CSDB::DB_Translate("Not connected to a database."));
				exit();
			}
		}
		...
		public function Connect($dsn, $username = false, $password = false, $options = array())
		{
			...
			{
				throw new \Exception(\CubicleSoft\CSDB::DB_Translate("The driver '%s' is invalid.  Must be '%s'.", htmlspecialchars($driver), htmlspecialchars($this->available_status)));
				exit();
			}
			...
			catch (Exception $e)
			{
				...
				if (is_resource($this->debug) || $this->debug)  throw new \Exception(\CubicleSoft\CSDB::DB_Translate("The connection to the database server failed.  %s", $e->getMessage()));
				else  throw new \Exception(\CubicleSoft\CSDB::DB_Translate("The connection to the database server failed."));
				exit();
			}
		...
		private function InternalQuery($params)
		{
			...
			else
			{
				...
				if (!$result["success"])
				{
					if ($result["errorcode"] == "skip_sql_query")  return new \CubicleSoft\CSDB_PDO_Statement($this, false, $filteropts);
					...
					throw new \Exception(\CubicleSoft\CSDB::DB_Translate("Error generating '%s' SQL query.  %s (%s)", $cmd, $result["error"], $result["errorcode"]));
					exit();
				}
			...
			if ($master && $this->mdsn !== false)
			{
				...
				{
					throw new \Exception(\CubicleSoft\CSDB::DB_Translate("The connection to the master database failed."));
					exit();
				}
			...
			foreach ($sqls as $num => $sql)
			{
				...
				if ($result === false)
				{
					...
					if (is_resource($this->debug) || $this->debug)  throw new \Exception(\CubicleSoft\CSDB::DB_Translate("%s %s (%s).  Error preparing SQL query:  %s  %s", $info[0], $info[2], $info[1], $sql, var_export($opts2, true)));
					else  throw new \Exception(\CubicleSoft\CSDB::DB_Translate("Error preparing SQL query.  %s %s (%s)", $info[0], $info[2], $info[1]));
					exit();
				}
				if (!$result->execute($opts2))
				{
					...
					if (is_resource($this->debug) || $this->debug)  throw new \Exception(\CubicleSoft\CSDB::DB_Translate("%s %s (%s).  Error executing SQL query:  %s  %s", $info[0], $info[2], $info[1], $sql, var_export($opts2, true)));
					else  throw new \Exception(\CubicleSoft\CSDB::DB_Translate("Error executing SQL query.  %s %s (%s)", $info[0], $info[2], $info[1]));
					exit();
				}
			...
			return new \CubicleSoft\CSDB_PDO_Statement($this, $result, $filteropts);
		}
		...
		{
			return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("The base class GenerateSQL() was called."), "errorcode" => "wrong_class_used");
		}
		...
		{
			return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("The base class ProcessColumnDefinition() was called."), "errorcode" => "wrong_class_used");
		}
		...
		protected function ProcessINSERT(&$master, &$sql, &$opts, $queryinfo, $args, $subquery, $supported)
		{
			...
			{
				if (!$supported["SELECT"])  return array("success" => false, \CubicleSoft\CSDB::DB_Translate("INSERT INTO SELECT not supported."), "insert_select_unsupported");
			...
			else  return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("INSERT command is missing required option or parameter."), "errorcode" => "missing_option_or_parameter");
		...
		protected function ProcessDELETE(&$master, &$sql, &$opts, $queryinfo, $args, $subquery, $supported)
		{
			...
			else
			{
				...
				foreach ($queryinfo as $key => $val)
				{
					if (is_int($key) && is_string($val) && strtoupper(substr($val, 0, 5)) === "WHERE")  return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("DELETE command appears to have a WHERE in a value instead of a key.  Query blocked to avoid an unintentional deletion of all records in the entire table.  Did you write 'WHERE something = ...' instead of 'WHERE' => 'something = ...'?"), "errorcode" => "query_blocked_where_clause");
				}
			}
		...
		protected function ProcessCREATE_TABLE(&$master, &$sql, &$opts, $queryinfo, $args, $subquery, $supported)
		{
			...
			if (isset($queryinfo["SELECT"]))
			{
				if (!isset($supported["AS_SELECT"]) || !$supported["AS_SELECT"])  return array("success" => false, \CubicleSoft\CSDB::DB_Translate("CREATE TABLE AS SELECT not supported."), "create_table_select_unsupported");
